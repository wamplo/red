...
#type-buy {
	...
	foreach ($data['posts'] as $posts) {
		...
		if ($posts['status'] == 0) {
			echo '<div class="l" style="margin-right: 10px;"><span id="type-news">Artikel</span></div>';
		}

		if ($posts['status'] == 1) {
			echo '<div class="l" style="margin-right: 10px;"><span id="type-sell">Penawaran</span></div>';
		}

		if ($posts['status'] == 2) {
			echo '<div class="l" style="margin-right: 10px;"><span id="type-buy">Permintaan</span></div>';
		}

		echo '<div class="l" style="width: 610px;"><a class="a" href="post?id='.$posts['PID'].'">'.$posts['title'].'</a> oleh <a href="'.$posts['username'].'" class="u">@'.$posts['name'].'</a> di <i><span id="group" class="gid-'.$posts['post_GID'].'">'.$posts['group'].'</span></i></div></li>';
	}
	
	?>
	</ul>
	</div>
	<div class="r" style="width: 245px;">
